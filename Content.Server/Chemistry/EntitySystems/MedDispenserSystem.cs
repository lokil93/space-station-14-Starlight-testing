...
namespace Content.Server.Chemistry.EntitySystems
{
    ...
    /// <seealso cref="MedDispenserComponent"/>
    /// </summary>
    [UsedImplicitly]
    public sealed class MedDispenserSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<MedDispenserComponent, ComponentStartup>(SubscribeUpdateUiState);
            SubscribeLocalEvent<MedDispenserComponent, SolutionContainerChangedEvent>(SubscribeUpdateUiState);
            SubscribeLocalEvent<MedDispenserComponent, EntInsertedIntoContainerMessage>(SubscribeUpdateUiState);
            SubscribeLocalEvent<MedDispenserComponent, EntRemovedFromContainerMessage>(SubscribeUpdateUiState);
            SubscribeLocalEvent<MedDispenserComponent, BoundUIOpenedEvent>(SubscribeUpdateUiState);

            SubscribeLocalEvent<MedDispenserComponent, MedDispenserSetDispenseAmountMessage>(OnSetDispenseAmountMessage);
            SubscribeLocalEvent<MedDispenserComponent, MedDispenserDispenseReagentMessage>(OnDispenseReagentMessage);
            SubscribeLocalEvent<MedDispenserComponent, MedDispenserClearContainerSolutionMessage>(OnClearContainerSolutionMessage);

            SubscribeLocalEvent<MedDispenserComponent, MapInitEvent>(OnMapInit, before: new []{typeof(ItemSlotsSystem)});
        }

        private void SubscribeUpdateUiState<T>(Entity<MedDispenserComponent> ent, ref T ev)
        {
            UpdateUiState(ent);
        }

        private void UpdateUiState(Entity<MedDispenserComponent> reagentDispenser)
        {
            ...
            var state = new MedDispenserBoundUserInterfaceState(outputContainerInfo, GetNetEntity(outputContainer), inventory, reagentDispenser.Comp.DispenseAmount);
        ...
        private List<ReagentInventoryItem> GetInventory(Entity<MedDispenserComponent> reagentDispenser)
        {
            var inventory = new List<ReagentInventoryItem>();

            for (var i = 0; i < reagentDispenser.Comp.NumSlots; i++)
            {
                var storageSlotId = MedDispenserComponent.BaseStorageSlotId + i;
        ...
        private void OnSetDispenseAmountMessage(Entity<MedDispenserComponent> reagentDispenser, ref ReagentDispenserSetDispenseAmountMessage message)
        {
        ...
        private void OnDispenseReagentMessage(Entity<MedDispenserComponent> reagentDispenser, ref ReagentDispenserDispenseReagentMessage message)
        {
        ...
        private void OnClearContainerSolutionMessage(Entity<MedDispenserComponent> reagentDispenser, ref ReagentDispenserClearContainerSolutionMessage message)
        {
            var outputContainer = _itemSlotsSystem.GetItemOrNull(reagentDispenser, SharedReagentDispenser.OutputSlotName);
            if (outputContainer is not { Valid: true } || !_solutionContainerSystem.TryGetFitsInDispenser(outputContainer.Value, out var solution, out _))
                return;
        ...
        private void ClickSound(Entity<MedDispenserComponent> reagentDispenser)
        {
        ...
        /// </summary>
        private void OnMapInit(EntityUid uid, MedDispenserComponent component, MapInitEvent args)
        {
